...
{
	Armeniaca_IsProcessExistA = (ARMENIACA_ISPROCESSEXISTA)::GetProcAddress(m_hModule, "Armeniaca_IsProcessExistA");
	if (!Armeniaca_IsProcessExistA)
	{
		return FALSE;
	}

	Armeniaca_IsProcessExistW = (ARMENIACA_ISPROCESSEXISTW)::GetProcAddress(m_hModule, "Armeniaca_IsProcessExistW");
	if (!Armeniaca_IsProcessExistW)
	{
		return FALSE;
	}

	Armeniaca_StartProcessA = (ARMENIACA_STARTPROCESSA)::GetProcAddress(m_hModule, "Armeniaca_StartProcessA");
	if (!Armeniaca_StartProcessA)
	{
		return FALSE;
	}

	Armeniaca_StartProcessW = (ARMENIACA_STARTPROCESSW)::GetProcAddress(m_hModule, "Armeniaca_StartProcessW");
